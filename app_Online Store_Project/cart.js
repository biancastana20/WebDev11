...
async function addToChart(id) {
	...
	let bookAlreadyInCart = cart.products.find(function (el) {
		return el.id == id;
	}); //verificam daca book ul pe care vrem sa facem entry in cart deja exista in cart
		...
		cart.products.map((product) => {
			containerCart.innerHTML += `
              <tr id='${product.id}'>
                 ...
                 <td><h5 class="admin-book-price">$${product.unitPrice}</h5></td>
                 <td>
                  ...
                  <td><p id="subtotal-${product.id}">$${product.subTotal}</p></td>
                  <td><a href="#" id="btn-remove-book" onclick="deleteBookFromCart('${product.id}')">Remove</a></td>
              </tr>`;
...
function deleteBookFromCart(id) {
	...
	cart.products = cart.products.filter(function (el) {
		return el.id != id;
	}); //va returna array cu elementele diferite de id
	updateCart(cart);
	document.getElementById('total').innerHTML = `Total: ${cart.total}`;
	document.getElementById(
		'productsNr'
	).innerHTML = ` Products: ${cart.productsNr}`;
}
...
function setQuantity(id, operation) {
	...
	cart.products = cart.products.map((el) => {
		if (el.id == id) {
			if (operation === '+') {
				el.quantity++;
			} else if (operation === '-' && el.quantity > 0) {
				el.quantity--;
			}
			el.subTotal =
				Math.round(+el.unitPrice * el.quantity * 100) / 100;
			document.getElementById('details-book-quantity-' + id).value =
				el.quantity;
			document.getElementById(
				'subtotal-' + id
			).innerHTML = `$${el.subTotal}`;
			//console.log(el)
		}
		return el;
	});
	...
	document.getElementById(
		'transport-total'
	).innerHTML = `Total transport: $${cart.transportTotal}`;
}
